...
def define_and_get_tornadovm_build_content(TORNADO_ROOT, TORNADO_SDK, PYTHON_HOME):
...
def generate_tornadovm_build_xml(TORNADO_ROOT, TORNADO_SDK, PYTHON_HOME):
    xml_build_content = define_and_get_tornadovm_build_content(TORNADO_ROOT, TORNADO_SDK, PYTHON_HOME)
...
def tornadovm_ide_init(tornadoSDKPath, javaHome, backends):
    ...
    backends = backends.split("=")[1]
    backend_profiles = process_backends_as_mvn_profiles(backends)
    ...
    generate_internal_maven_build_xml(tornadoRoot, javaHome, backend_profiles)
    generate_tornadovm_build_xml(tornadoRoot, tornadoSDKPath, pythonHome)
