 ...
 *
 */
package uk.ac.manchester.tornado.benchmarks.matrixTranspose;

import static uk.ac.manchester.tornado.api.math.TornadoMath.abs;
import static uk.ac.manchester.tornado.benchmarks.LinearAlgebraArrays.matrixTranspose;
 ...
 * </p>
 * <code>
 * tornado -m tornado.benchmarks/uk.ac.manchester.tornado.benchmarks.BenchmarkRunner matrixTranspose
 * </code>
 */
public class MatrixTransposeTornado extends BenchmarkDriver {
    ...
    public MatrixTransposeTornado(int iterations, int size) {
        super(iterations);
        this.size = size;
    }
    ...
    public void setUp() {
        ...
        taskGraph.task("matrixTranspose", LinearAlgebraArrays::matrixTranspose, a, b, size);
    ...
    public boolean validate(TornadoDevice device) {
        ...
        matrixTranspose(a, result, size);

        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                if (abs(result.get((i * size) + j) - b.get((i * size) + j)) > 0.01) {
                    val = false;
                    break;
                }
            }
