...
public class MetalBlockVisitor implements ControlFlowGraph.RecursiveVisitor<HIRBlock> {

    MetalCompilationResultBuilder metalBuilder;
    MetalAssembler asm;
    Set<HIRBlock> merges;
    ...
    public MetalBlockVisitor(MetalCompilationResultBuilder resBuilder) {
        this.metalBuilder = resBuilder;
    ...
    private void emitBeginBlockForSwitchStatements(HIRBlock dom, HIRBlock beginBlockNode) {
        ...
        if (defaultSuccessorIndex == caseIndex) {
            asm.emit(MetalAssemblerConstants.DEFAULT_CASE + MetalAssemblerConstants.COLON);
        } else {
            for (Integer idx : commonCases) {
                asm.emit(MetalAssemblerConstants.CASE + " ");
                JavaConstant keyAt = switchNode.keyAt(idx);
                asm.emit(keyAt.toValueString());
                asm.emit(MetalAssemblerConstants.COLON);
                asm.emitLine("");
                asm.indent();
            }
        }
    ...
    public HIRBlock enter(HIRBlock block) {
        ...
        if (block.isLoopHeader()) {
            loopCount++;
            metalBuilder.emitLoopBlock(block);
        } else {
    ...
    private void closeSwitchStatement(HIRBlock block) {
        asm.emitLine(MetalAssemblerConstants.BREAK + MetalAssemblerConstants.STMT_DELIMITER);
    ...
    public void exit(HIRBlock block, HIRBlock value) {
        ...
        if (block.isLoopEnd()) {
            ...
            if (metalBuilder.shouldRemoveLoop()) {
         ...
         */
        metalBuilder.emitRelocatedInstructions(block);
    }
